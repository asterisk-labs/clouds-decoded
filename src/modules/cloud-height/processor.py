...
# Stream handler for terminal logs
...
def configure_file_logging(log_dir):
    """
    Configures file logging by adding a FileHandler to the module-level logger.
    """
    os.makedirs(log_dir, exist_ok=True)
    file_handler = logging.FileHandler(os.path.join(log_dir, 'cloud_height.log'), mode='w')
...
class CloudHeightProcessor:
    def __init__(self, config: CloudHeightConfig):
        ...
        logger.info(f"Scene latitude: {self.scene.latitude}")
    ...
    def _normalizePatch(self, patch, remove_mean=True, remove_var=True):
        ...
        nan_check = np.any(
            np.isnan([patch[0, 0], patch[0, -1], patch[-1, 0], patch[-1, -1]]))
    ...
    def _correlateAtHeight(self, footprint_bands, centre, along_track_size, height, direction, remove_mean=True, remove_var=True):
        ...
        offsets = heightsToOffsets(
            [height] * len(footprint_bands), footprint_bands.keys(), pixel_size=pixel_size)
    ...
    def _processColumn(self, column, brightness_mask=True):
        ...
        along_track_size = self.config.convolved_size_along_track // self.config.along_track_resolution
        gradient_power_ratio = self.config.along_track_resolution / \
            self.config.across_track_resolution

        # Create a brightness mask if required
        mask = column.getMask(self.config.threshold_band,
                              self.config.cloudy_thresh) if brightness_mask else None
        ...
        if self.config.target_features == "reflectance":
            ...
            raise ValueError(
                f"Invalid target_features: {self.config.target_features}")

        # Determine the range of centers to process based on max parallax offset
        max_offset_val = heightsToOffsets([self.config.max_height] * len(
            target_features), target_features.keys(), self.config.along_track_resolution)
        max_offset = int(np.ceil(max_offset_val.max()))

        shape_0 = column.bands[self.config.reference_band].shape[0]
        if column.direction == 'up':
            centres = np.arange(along_track_size // 2, shape_0 -
                                max_offset - along_track_size // 2, along_track_stride)
        else:
            centres = np.arange(along_track_size // 2 + max_offset,
                                shape_0 - along_track_size // 2, along_track_stride)
        ...
        for centre, coord in zip(centres, extracted_coords):
            ...
            scores = self._correlateAtHeights(
                target_features, centre, column.direction)
    ...
    def _worker_job(self, data_queue: multiprocessing.Queue, result_queue: multiprocessing.Queue):
        ...
        while True:
            ...
            try:
                ...
                retrievals, retrieved_coords = self._processColumn(
                    col, brightness_mask=True)
                result_queue.put(
                    {'retrievals': retrievals, 'coords': retrieved_coords})
            except Exception as e:
                # log exception and send a concise error message to the main process
                logger.exception("Worker failed processing %s",
                                 os.path.basename(path))
                result_queue.put(
                    {'error': f"Worker failed on {os.path.basename(path)}: {e}"})
            finally:
                if os.path.exists(path):
                    try:
                        ...
                        logger.debug(
                            "Failed to remove temporary file %s", path, exc_info=True)
    ...
    def process(self):
        ...
        with tempfile.TemporaryDirectory(dir="/dev/shm") as temp_dir:
            logger.info("Using temporary directory for this run: %s", temp_dir)
            ...
            if self.config.smoothing_mode == 'spatial':
                ...
                heights_buffer = np.zeros(
                    (max_points, N_heights), dtype=np.float32)
            elif self.config.smoothing_mode == 'independent':
                heights_buffer = np.zeros(max_points, dtype=np.float32)
            else:
                raise ValueError(
                    f"Invalid smoothing mode: {self.config.smoothing_mode}. Choose 'spatial' or 'independent'.")
            ...
            workers = []
            logger.info("Spawning %d worker processes...",
                        self.config.n_workers)
            for _ in range(self.config.n_workers):
                p = multiprocessing.Process(target=self._worker_job, args=(
                    column_iterator.queue, result_queue))
                workers.append(p)
                p.start()
            ...
            with tqdm(total=total_columns, desc="Processing Columns", smoothing=0.05) as pbar:
                for _ in range(total_columns):
                    ...
                    if 'error' in result:
                        logger.error("Error from worker: %s", result['error'])
                        pbar.update(1)
                        continue
                    ...
                    if retrievals and len(retrievals) > 0:
                        ...
                        if count + num_retrieved > max_points:
                            raise ValueError(
                                "Exceeded preallocated space for results.")
                        heights_buffer[count:count +
                                       num_retrieved] = retrievals
                        coords_buffer[count:count +
                                      num_retrieved] = result.get('coords')
                        count += num_retrieved
                    pbar.update(1)
    ...
    def postprocess(self):
        ...
        """
        logger.info("Starting post-processing: Regridding and Smoothing...")
        logger.info(f"Number of valid retrievals: {len(self.final_heights)}")

        retrievalcube = RetrievalCube(
            self.final_heights, self.final_coords, self.config)
        ...
        grid_x, grid_y = np.meshgrid(
            np.arange(0, 109800, grid_stride), np.arange(0, 109800, grid_stride))
        ...
        with tqdm(total=len(grid_points), desc="Smoothing points") as pbar:
            for point in grid_points:
                coords, scores = retrievalcube.queryRadius(
                    point, smoothing_sigma * 2)

                if len(coords) == 0:
                    smoothed_scores_list.append(
                        np.nan * np.ones_like(self.config.heights))
                    pbar.update(1)
                    continue
                ...
                if np.min(dists) < smoothing_sigma * 1.5:
                    ...
                    weights = np.exp(-0.5 *
                                     (dists / smoothing_sigma)**2) * confs

                    if np.sum(weights) > 0:
                        weights /= np.sum(weights)
                        smoothed_value = np.sum(
                            scores * weights[:, np.newaxis], axis=0)
        ...
        final_gridded_heights[final_gridded_coords_stride[:, 1],
                              final_gridded_coords_stride[:, 0]] = self.final_heights
    ...
    def plot_results(self):
        ...
        rgb = np.stack([rgb_bands['B04'], rgb_bands['B03'],
                       rgb_bands['B02']], axis=-1)
        ...
        ax.imshow(rgb, extent=[0, rgb.shape[1] *
                  resolution, 0, rgb.shape[0] * resolution])

        # use gridded heights if available
        heights_log = np.log10(np.clip(self.final_gridded_heights, 500, 16000))
        height_map = ax.imshow(heights_log, cmap='gist_ncar', alpha=0.33, extent=[
                               0, rgb.shape[1] * resolution, 0, rgb.shape[0] * resolution])
        ...
        if self.config.plot_writeto:
            logger.info("Saving plot to %s", self.config.plot_writeto)
        ...
        height_map = ax.imshow(heights_log, cmap='prism', alpha=0.33, extent=[
                               0, rgb.shape[1] * resolution, 0, rgb.shape[0] * resolution], vmin=0, vmax=50000)
        cbar = plt.colorbar(height_map, ax=ax, fraction=0.046, pad=0.04)
        ...
        if self.config.plot_writeto:
            outpath = self.config.plot_writeto.replace('.png', '_linear.png')
    ...
    def save_results(self):
        ...
        if self.final_heights is not None and self.final_coords is not None:
            try:
                np.savez_compressed(config.pcloud_writeto, heights=self.final_heights,
                                    coords=self.final_coords, grid=self.final_gridded_heights)
                logger.info("Results saved to %s", config.pcloud_writeto)
            except Exception:
                logger.exception("Failed to save results to %s", config.pcloud_writeto)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', '-s', action='store_true',
                        help='Save output as .npz')
    parser.add_argument('--plot', '-p', action='store_true',
                        help='Plot the heights')
    ...
    if args.log:
        os.makedirs(config.log_writeto, exist_ok=True)
        configure_file_logging(config.log_writeto)
        # Storing config for reproducibility
        args_yaml_path = os.path.join(config.log_writeto, 'args.yaml')
        config_path = os.path.join(config.log_writeto, 'config.yaml')
        os.system(f"cp {args.config} {config_path}")
        args_dict = vars(args)
        with open(args_yaml_path, 'w') as yaml_file:
            yaml.dump(args_dict, yaml_file, default_flow_style=False)
        logger.info("Arguments saved to %s", args_yaml_path)

    logger.info('Starting cloud height algorithm with the following configuration:')
    ...
    if args.plot:
        os.makedirs(os.path.dirname(config.plot_writeto), exist_ok=True)
        processor.plot_results()

    if args.save:
        os.makedirs(os.path.dirname(config.pcloud_writeto), exist_ok=True)
