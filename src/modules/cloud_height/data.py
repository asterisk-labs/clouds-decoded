...
class ColumnExtractor:
    ...
    def getMaskInterpolator(self, mask):
        ...
        Assumes it matches the resolution of the reference band (usually 10m).
        """
        # Assuming mask is 2D numpy array
        # Resolution needs to be inferred or passed. 
        # Typically the mask generated by cloud_mask module uses Sentinel2Scene transform, 
        # so it matches the reference band resolution (10m).
        res = getattr(self, 'original_resolution', 10)
        
        return RegularGridInterpolator((
                np.arange(mask.shape[0]) * res,
                np.arange(mask.shape[1]) * res
            ),
            mask,
